...
# ────────── 工具函数 ──────────
def clamp(v, min_=1.0): return max(min_, v)

def choose_upper(pool, target, tol=0.5, max_items=5):
    """符合“打底≥1；若要层叠必须三层全”的上身组合"""
    ...
    for r in range(1, max_items+1):
        for combo in itertools.combinations(items, r):
            ...
            if not cats & BASE_CATS:  # 必须有打底
                continue
            # 统计层数
            n_base   = len([c for c in cats if c in BASE_CATS])
            n_outer  = len([c for c in cats if c in JACKET_CATS])
            n_mid    = len([c for c in cats if c in MID_CATS])
            dup_layer = (n_outer>1) or (n_mid>1)
            if dup_layer and not (n_outer>=1 and n_mid>=1):
                continue  # 想层叠→必先三层齐
...
if go:
    ...
    # 精准 + 微调 ±0.5
    plans = [mk_plan(u_t,d_t,"精准匹配")]
    shift = random.choice((0.5,-0.5))
    plans.append(mk_plan(clamp(u_t+shift),clamp(d_t+shift),f"微调 {shift:+.1f}"))
    if prefer=="怕冷": plans[-1]=mk_plan(u_t+1,d_t+0.5,"加衣版")
    if prefer=="怕热": plans[-1]=mk_plan(clamp(u_t-1),clamp(d_t-0.5),"减衣版")
